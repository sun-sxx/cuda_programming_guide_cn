...
同样，这比使用 ``cudaDeviceSynchronize()`` 更可取、更聚焦。调用 ``cudaEventQuery()`` 并检查其返回值（例如在轮询循环中）将是一种非阻塞的替代方案。
...
流优先级范围用 ``[最高优先级，最低优先级]`` 表示， 可以使用 ``cudaDeviceGetStreamPriorityRange()`` 函数获取。
