...
.. code-block:: c++
   ...
   namespace cuda {   // "nv" 和 "cooperative_groups" 命名空间同理
   ...
   namespace utils {
     namespace cuda {
       // 正确，命名空间 "cuda" 可以嵌套在非保留命名空间内使用
       void bar();
     } // namespace cuda
   } // namespace utils

   // 错误，等同于在全局作用域向命名空间 "cuda" 添加符号
...
在单独编译模式下，如果 ``__device__`` 或 ``__global__`` 函数定义需要存在于特定翻译单元中，则该函数的参数和返回类型在该翻译单元中必须是完整的（即 ODR-use）。
...
.. code-block:: c++
   ...
   __device__ auto* ptr = foo; // ODR-use，取地址
...
.. code-block:: text
   ...
   nvlink error   : Prototype doesn't match for '_Z3foo1S' in '...first.o',
                    first defined in '...second.o'
...
- 不能有可变数量的参数，即 C 省略号语法 ``...`` 和 ``va_list`` 类型。C++11 可变参数模板在满足相关限制条件下是允许的。
- 函数参数通过常量内存传递给设备，总大小限制为 32,764 字节。
- 函数参数不能是 ``std::initializer_list`` 类型。
- 多态类参数（ ``virtual`` ）被视为未定义行为。
- Lambda 表达式和闭包类型在满足相关限制条件下是允许的。
...
从主机代码启动 ``__global__`` 函数时，每个参数类型可以是非平凡可复制或非平凡可析构的。但是，对这些类型的处理不遵循标准 C++ 模型，如下所述。用户代码必须确保此工作流不会影响程序正确性。该工作流在两个方面与标准 C++ 存在差异：

1. **原始内存复制代替拷贝构造函数调用**

   CUDA Runtime 通过复制原始内存内容将 kernel 参数传递给 ``__global__`` 函数，最终使用 ``memcpy`` 。如果参数是非平凡可复制的并提供了用户定义的拷贝构造函数，则在主机到设备的复制过程中会跳过该拷贝构造函数的调用及其副作用。
   ...
   .. code-block:: c++
      ...
      __global__ void device_function(MyStruct my_struct) {
          // 此断言失败，因为 "my_struct" 是通过复制原始内存内容获得的，
          // 拷贝构造函数被跳过。
...
2. **析构函数可能在 ``__global__`` 函数完成之前被调用**
   ...
   Kernel 启动与主机执行是异步的。因此，如果 ``__global__`` 函数参数具有非平凡析构函数，析构函数可能在主机代码中甚至在 ``__global__`` 函数完成执行之前就已执行。这可能会破坏析构函数具有副作用的程序。
   ...
   .. code-block:: c++
      ...
      __global__ void device_function(MyStruct my_struct) {
          assert(var == 0); // 失败，MyStruct::~MyStruct() 将值设为 3
      }

      int main() {
          MyStruct my_struct;
          // GPU kernel 执行与主机执行是异步的。
          // 因此，MyStruct::~MyStruct() 可能在 kernel 完成执行之前执行。
