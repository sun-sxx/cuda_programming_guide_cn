.. _async-copies-pipeline:

.. _async-copies-shared-memory-barrier:

.. _async-copies-ptx-docs:
...
在此示例中，我们将演示如何使用 TMA 从 Global 内存预取数据到共享内存。在迭代拷贝和计算模式中，这允许用当前迭代的计算隐藏未来迭代的数据传输延迟，可能增加飞行中的字节数。
